...
def _get_paths_from_mat(dataroot):
    '''get image path list from lmdb meta info'''
    meta_info = h5py.File(os.path.join(dataroot), 'r')
    key = meta_info.keys()[0]
    sizes = meta_info[0][-2::].size()
    return key, sizes
...
def get_image_paths(data_type, dataroot):
    ...
    if dataroot is not None:
        if data_type == 'mat':
            paths, sizes = _get_paths_from_mat(dataroot)
        elif data_type == 'img':
